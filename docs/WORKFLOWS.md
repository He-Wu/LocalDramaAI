...
`build_render_timeline` reads the project graph in deterministic Scene/Shot/Dialogue order and binds persisted values to measured source bytes. `cues_from_timeline` generates sequential, non-overlapping millisecond cues within the frame-derived timeline; `serialize_srt` writes deterministic UTF-8 CRLF SubRip after removing control/style injection.

`FFmpegRenderProvider` normalizes every selected Shot to the locked CFR profile, reconstructs finite audio from ordered Dialogue WAVs (silence when absent), concatenates visuals, conditionally burns subtitles, strictly probes/full-decodes the result, and atomically publishes an immutable MP4 and provider manifest. `render_project` rechecks the snapshot under a project publication lock and short SQLite write transaction, persists SUBTITLE and FINAL_VIDEO Assets/manifests, and updates Project pointers without modifying upstream Shot links/statuses. The canonical `output/final.mp4` alias is rebuilt from the authoritative immutable Asset and must be `READY` for acceptance.
